...
app.post('/register', async (req, res) => {
    ...
    console.log('Registrando usuario recibido:', { username, password });
    try {
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length > 0) {
            ...
            console.log('No se encontró usuario con:', { username, password });
...
app.get('/messages/:sender/:receiver', async (req, res) => {
    ...
    console.log('Obteniendo mensajes entre:', { sender, receiver });
    try {
        const result = await pool.query(
...
app.post('/groups', async (req, res) => {
    ...
    console.log('Creando grupo:', { group_name, members, creator });
    if (!group_pic) {
...
app.post('/group_messages', async (req, res) => {
    ...
    console.log('Enviando mensaje a grupo:', { group_id, sender, text });
    try {
        const result = await pool.query(
