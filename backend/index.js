...
wss.on('connection', (ws, req) => {
    ...
    const username = urlParams.get('username'); // Respetamos el caso exacto
    if (username) {
...
app.post('/register', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            ...
            [username, password] // Guardamos exactamente como se ingresa
        );
...
app.post('/login', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            ...
            [username, password] // Sensible a mayúsculas
        );
...
app.post('/profile-pic', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            ...
            [profilePic, username] // Sensible a mayúsculas
        );
        if (result.rows.length > 0) {
...
app.get('/profile-pic/:username', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            ...
            [username] // Sensible a mayúsculas
        );
        if (result.rows.length > 0) {
...
app.post('/contacts', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            ...
            [username, contact] // Guardamos exactamente como se ingresa
        );
...
app.get('/contacts/:username', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            ...
            [username, limit, offset] // Sensible a mayúsculas
        );
...
app.post('/messages', async (req, res) => {
    ...
    try {
        const contactCheck = await pool.query(
            ...
            [receiver, sender] // Sensible a mayúsculas
        );
        if (contactCheck.rows.length === 0) {
            await pool.query(
                'INSERT INTO contacts (username, contact) VALUES ($1, $2) ON CONFLICT DO NOTHING',
                [receiver, sender] // Guardamos exactamente como se ingresa
            );
        ...
        const result = await pool.query(
            ...
            [sender, receiver, text] // Guardamos exactamente como se ingresa
        );
...
app.get('/messages/:sender/:receiver', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            ...
            [sender, receiver] // Sensible a mayúsculas
        );
...
app.post('/groups', async (req, res) => {
    ...
    try {
        const groupResult = await pool.query(
            ...
            [group_name, creator, group_pic] // Guardamos exactamente como se ingresa
        );
        ...
        for (const member of allMembers) {
            await pool.query(
                ...
                [group_id, member] // Guardamos exactamente como se ingresa
            );
        }
...
app.post('/group_messages', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            ...
            [group_id, sender, text] // Guardamos exactamente como se ingresa
        );
        const message = result.rows[0];
        notifyMessage(message);
        res.json(message);
...
app.post('/group-pic', async (req, res) => {
    ...
    try {
        const creatorCheck = await pool.query(
            ...
            [group_id, username] // Sensible a mayúsculas
        );
        if (creatorCheck.rows.length === 0) {
            console.log('Grupo no encontrado o usuario no autorizado:', { group_id, username });
...
app.post('/group-add-member', async (req, res) => {
    ...
    try {
        ...
        const memberExists = await pool.query(
            'SELECT 1 FROM users WHERE username = $1',
            [new_member] // Sensible a mayúsculas
        );
        ...
        await pool.query(
            'INSERT INTO group_members (group_id, username) VALUES ($1, $2) ON CONFLICT DO NOTHING',
            [group_id, new_member] // Guardamos exactamente como se ingresa
        );
...
app.post('/group-remove-member', async (req, res) => {
    ...
    try {
        ...
        await pool.query(
            ...
            [group_id, member_to_remove] // Sensible a mayúsculas
        );
